...
from PyQt5.QtWidgets import QApplication, QLabel, QMainWindow, QMenu, QToolBar, QToolButton, \
    ...
    QStatusBar, QGridLayout, QDial,QToolBox,QRadioButton, QGroupBox, QSpinBox,QTabWidget, QComboBox,QPlainTextEdit,QInputDialog
...
from pyLEFA_Functions import gray2binary,detectPLineHough,detectPLineHough2,uniteLines2,lineKB,createSHPfromDictionary,\
                            saveLinesShpFile,saveLinesShpFile2,rasterize_shp,rasterize_shp2,saveGeoTiff,get_minkowski,\
                            get_pixels_sum,build_rose_diag
    ...
    def open_data(self,filepath,parent_obj=None,content=None):
        ...
        if 'tif' in os.path.split(filepath)[-1]:
            try:
                ...
                dobj = Dataobj(id=self.get_new_id(), gdal_obj=gdal_obj,file_path=filepath,ftype='tif',parent_obj=parent_obj,content=content)
                self.add_obj(dobj)
            except:
            ...
            dobj = Dataobj(id=self.get_new_id(), gdal_obj=None, file_path=filepath, ftype='shp',parent_obj=parent_obj,content=content)
            self.add_obj(dobj)
...
class Window(QMainWindow):
    ...
    def load_project(self,path):
        ...
        for key in [*somedict['body']]:
            self.add_data_obj(somedict['body'][key]['file_path'],parent_obj=somedict['body'][key]['parent_obj'])
    ...
    def _createToolBars(self):
        ...
        new_project_button_icon.addPixmap(QtGui.QPixmap(new_project_button_icon_path), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def selectFileForAnyAnalysis(self,ftype='tif', type_analysis='line', txt='Select file'):
        print('Select file for analysis was selected')
        #self.select_file_analysis = selectFileWindow(parent = self)
        self.current_method = type_analysis
        self.select_file_analysis = selectFileWindow(parent=self,ftype=ftype)
        self.select_file_analysis.show()

    #this !!!function runs any type of analysis from file selection window on Ok
    def do_analysis(self):
        print(self.current_method,'method of analysis for', self.file_for_analysis, 'has been activated')
        ...
        if self.current_method == 'line':
            ...
            if save_shp_path:
                ...
                imBW = gray2binary(self.get_matrix_by_name_id(name=self.file_for_analysis),
                                   method=self.settings.value('data')['binarization'],sigma=self.settings.value('data')['line_sigma'])
                ...
                gdal_obj = self.get_gdal_by_name_id(name=self.file_for_analysis)
                #save line to shp
                saveLinesShpFile2(lines,save_shp_path,gdal_obj)
                #reopen file to project
                #TODO specify parent object
                self.add_data_obj(save_shp_path,parent_obj=self.file_for_analysis, content='line')

        elif self.current_method == 'rose':
            txt = self.language_dict['commands']['rose_label'][self.selected_language]
            new_lines = self.get_features_by_name_id(name=self.file_for_analysis)
            build_rose_diag(new_lines,txt=txt+' '+self.file_for_analysis)
            ...
            if save_shp_path:
                ...
                parent_obj = self.get_parent_by_name_id(name=self.file_for_analysis)
                ...
                if gdal_obj:
                    ...
                    lines2 = self.get_features_by_name_id(name=self.file_for_analysis)
                    extent = self.get_extent_by_name_id(name=self.file_for_analysis)
                    ...
                    ext = [[extent[0],extent[3]],[extent[0],extent[2]],[extent[1],extent[2]],[extent[1],extent[3]]]
                #test raster save
                #TODO saves tif only if parent is known
                if self.get_parent_by_name_id(name=self.file_for_analysis):
    ...
    def datastorage2xml(self,path):
        ...
        for obj in self.data_storage.get_objects():
            ...
            <type>{obj.type}</type><parent>{obj.parent}</parent>
    ...
    def file_open_dialogue(self, type_filter='tif'):
        ...
        fileName = QFileDialog.getOpenFileName(self, ("Open File"), '', name_mask)

        if fileName[0] != '':
            #TODO add data object opened from disk
            self.add_data_obj(fileName[0])
...
class selectFileWindow(QWidget):
    ...
    def on_ok(self):
        ...
        try:
            ...
            self.parent.msg_info(self.parent.language_dict['commands']['msg_specify_file'][self.parent.selected_language])
            return None
        ...
        msgBox = QMessageBox.question(self, txt_title, txt_question.replace('#',item_text),
                                       QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if msgBox == QMessageBox.Yes:
...
class Preferences(QWidget):
   def __init__(self,parent=None):
       ...
       self.radioCanny = QRadioButton(self.parent.language_dict['commands']['line_det_bin_canny'][self.parent.selected_language])
       self.radioFlows = QRadioButton(self.parent.language_dict['commands']['line_det_bin_flows'][self.parent.selected_language])
       #sigma for Canny analysis parameters
       self.sigmaCannyLabel = QLabel(self.parent.language_dict['commands']['line_det_bin_canny_sigma'][self.parent.selected_language])
       ...
       binaryMethodLabel = QLabel(self.parent.language_dict['commands']['line_det_bin_label'][self.parent.selected_language])
       houghTransformLabel = QLabel(self.parent.language_dict['commands']['line_det_hough_label'][self.parent.selected_language])
       houghTransformTreshLabel = QLabel(self.parent.language_dict['commands']['line_det_treshold_label'][self.parent.selected_language])
       houghTransformLenLabel = QLabel(self.parent.language_dict['commands']['line_det_len_label'][self.parent.selected_language])
       houghTransformGapLabel = QLabel(self.parent.language_dict['commands']['line_det_gap_label'][self.parent.selected_language])
       ...
       self.spinRasterRes.setRange(10, 5000)
       ...
       presetsLineamLabel = QLabel(self.parent.language_dict['commands']['line_load_preset_label'][self.parent.selected_language])
       #self.presetsLineamCombo.addItems(['small', 'medium', 'many'])
       apply_preset_btn = QPushButton(self.parent.language_dict['commands']['line_apply_preset'][self.parent.selected_language])
       ...
       tab_fdens_name = self.parent.language_dict['commands']['density_fractal_set_tab_name'][self.parent.selected_language]
       densWinSizeLabel = QLabel(self.parent.language_dict['commands']['density_win_size_label'][self.parent.selected_language])
       fracWinSizeLabel = QLabel(self.parent.language_dict['commands']['fractal_win_size_label'][self.parent.selected_language])
...
class Layers(QWidget):
    ...
    def initUI(self):
        ...
        try:
            self.setWindowTitle(self.parent.language_dict['commands']['layers_window_title'][self.parent.selected_language])
        except:
            self.setWindowTitle('Project files:')
        #buttons in layers list
        try:
            self.label = QLabel(self.parent.language_dict['commands']['layers_window_label'][self.parent.selected_language])
        except:
            self.label = QLabel('Project files:')

        self.layer_list = QListWidget()
        try:
            self.layer_button = QPushButton(self.parent.language_dict['commands']['layers_window_show_stat_tbn'][self.parent.selected_language])
            self.layer_remove = QPushButton(self.parent.language_dict['commands']['layers_window_remove_layer_tbn'][self.parent.selected_language])
            self.layer_show = QPushButton(self.parent.language_dict['commands']['layers_window_show_layer_tbn'][self.parent.selected_language])
        except:
    ...
    def onClick(self, event):  # click on app window
        ...
        try:
            if len(self.parent.layer_list.selectedItems()) == 0:
                self.parent.layers.layer_list.setCurrentItem(self.parent.layers.layer_list.item(0))  # set selection to first items
            else:
